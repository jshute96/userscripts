...
const KeyboardShortcuts = (function () {
  ...
  function parseSpec(spec) {
    ...
    // e.key for the space bar is ' ', which a spec can't hold readably.
    if (key === 'space') key = ' ';
  ...
  function matches(b, e) {
    ...
    // Shift is enforced for letters, where 'i' vs 'shift-i' is a real
    // distinction and we want the *modifier*, not the character case,
    // to decide (Caps Lock + i must not read as Shift-I), and for named
    // keys like 'arrowright' and 'space', where 'shift-arrowright' must
    // not also fire on a plain arrow, nor a 'space' binding take the
    // browser's Shift+Space (scroll up). Symbol keys like '?' are produced *by*
    // shift on most layouts, so enforcing shift:false there would make
    // them unbindable.
    if ((LETTER.test(b.key) || b.key.length > 1 || b.key === ' ') &&
        e.shiftKey !== b.shift) {
      return false;
    }
    return true;
  ...
  // Named keys as they're printed on the keyboard, for the help screen.
  const KEY_NAMES = {
  ...
  function displaySpec(b) {
    ...
    if (KEY_NAMES[b.key]) {
      parts.push(KEY_NAMES[b.key]);
    } else if (b.key.length > 1) {
      parts.push(b.key[0].toUpperCase() + b.key.slice(1));
    } else {
