...
const CommentNav = (function () {
  ...
  // How far a jump to the first comment may still want to carry us
  // before we stop calling ourselves "above the list" — see
  // `findCurrent`. A smooth scroll settles a pixel or so off its
  // target and a site's header offset is a fractional measurement, so
  // an exact comparison would call a comment we just landed on "not
  // reached yet". Same value and same meaning as the settle scroller's
  // `DRIFT_TOLERANCE_PX` ("close enough to count as landed"); they're
  // kept separate only because one is measured in scroll distance and
  // the other in the target's viewport position.
  const ANCHOR_SLOP_PX = 4;
  ...
  function makeSettleScroller(tag) {
    ...
    // `offsetOf` is a function, not a number: a correction is itself a
    // new scroll, and on a site whose header appears and disappears the
    // right offset can differ from the one the original scroll used.
    // Correcting a 12px overshoot means scrolling *up* 12px, which on
    // Substack slides the header back in — so the destination that was
    // right a moment ago now has 72px of header over it.
    function scroll(el, offsetOf) {
      const mine = ++token;
      scrollWindow(el, offsetOf());
      ...
      function tick() {
        ...
        if (performance.now() < deadline) {
          ...
          console.log(tag, 'scroll settle timed out at',
            Math.round(el.getBoundingClientRect().top - offsetOf()) + 'px');
        }
      }
      requestAnimationFrame(tick);
    }
  ...
  function create(spec) {
    ...
    function scrollToEl(el) {
      // `el` is passed so a site can vary the offset by where the jump
      // is going. Substack's top bar, for one, hides when the page
      // scrolls down and reappears when it scrolls up, so the height to
      // reserve depends on the direction of this particular jump. Sites
      // whose header just sits there ignore the argument.
      const offsetOf = () => headerOffset(el);
      const offset = offsetOf();
      ...
      switch (strategy) {
        case 'settle':    return settler.scroll(el, offsetOf);
    ...
    // How far, and in which direction, a jump to `el` would actually
    // scroll: positive means downward (`el` is below where a jump would
    // park it), negative upward, ~0 means we're already there or can't
    // get closer. Mirrors `scrollToEl`'s arithmetic and clamps to the
    // scrollable range, so it answers for the scroll that would really
    // happen rather than the one we'd ask for.
    function jumpDistance(el) {
      const offset = headerOffset(el);
      const container = containerOf();
      if (container && (strategy === 'container' || strategy === 'raf')) {
        const target = container.scrollTop
          + el.getBoundingClientRect().top
          - container.getBoundingClientRect().top - offset;
        const max = container.scrollHeight - container.clientHeight;
        return Math.min(Math.max(target, 0), max) - container.scrollTop;
      }
      const target = el.getBoundingClientRect().top + window.scrollY - offset;
      const max = document.documentElement.scrollHeight - window.innerHeight;
      return Math.min(Math.max(target, 0), max) - window.scrollY;
    }
    ...
    // Bumping `anchorToken` cancels a pending after-open re-anchor;
    // see `anchorAfterOpen`. Anything that invalidates the remembered
    // jump target is the user taking over the viewport, and that must
    // also call off a scroll we were about to make on their behalf.
    let anchorToken = 0;
    ...
    function findCurrent(all) {
      ...
      // No element here: this asks "what's covered right now?", not
      // "what will be covered after a jump", so a site with a
      // show/hide header should report what's actually on screen.
      const anchor = bounds.top + headerOffset();

      // Positioned above the whole list: a jump to the first comment
      // would still carry us *downward*, so there is no comment we've
      // arrived at yet. Reporting none is what makes the downward keys
      // land on the first comment here rather than stepping past it to
      // the second — the case where a comments pane or section opens at
      // its top with the first comment already in view, some way down.
      // The upward keys correctly find nowhere to go.
      //
      // Asked as "would a jump move us, and which way?" rather than
      // "where is the first comment on screen?" because only the former
      // survives the two ways the geometric question misleads:
      //
      //   * A jump parks the comment, not its body, and a body starts
      //     20-30px below its comment's top (avatar, name, timestamp).
      //     Measuring the body read a comment we had just landed on as
      //     still unreached, so the next key re-jumped to it instead of
      //     advancing — invisible for `j` after `j`, which rides
      //     `lastJumpTarget`, but `c` clears that deliberately, so the
      //     first `j` after `c` did nothing.
      //   * The scroll may not be *able* to reach the anchor. The Verge's
      //     drawer scrolls 144px in total, and on a short thread its
      //     first comment sits ~500px down and simply cannot come up, so
      //     a geometric test stays true forever and every `j` after a
      //     `c` costs a wasted press.
      //
      // Clamping the destination the way the scroller does is what
      // catches that second case: a jump that can't move reads as no
      // jump, and the comment counts as reached.
      if (jumpDistance(all[0]) > ANCHOR_SLOP_PX) return null;

      const gate = anchor + MIN_VISIBLE_PX;
      for (const el of all) {
    ...
    // Most `open` controls just reveal a panel that is already at its
    // top, so `c` opens and the next `c` anchors — two presses, and
    // deliberately so. But some of them *scroll the page themselves*
    // (The Athletic's "Open Comments" pill does), and where that lands
    // depends on how much content loaded in on the way: measured on one
    // article, the comments banner ended up 651px above the viewport
    // while the document grew from 9417px to 15350px. Either direction
    // leaves `c` looking like it missed, and the library never scrolled
    // at all, so no scroll strategy can help.
    //
    // Sites in that shape set `open.anchorAfterOpen`, and once the
    // anchor turns up we scroll to it ourselves. Pair it with
    // `strategy: 'settle'` where the page is still growing, or the
    // re-anchor inherits the same undershoot it's correcting.
    const ANCHOR_WAIT_MS = 5000;
    const ANCHOR_POLL_MS = 150;

    function anchorAfterOpen() {
      const mine = ++anchorToken;
      const deadline = performance.now() + ANCHOR_WAIT_MS;
      (function poll() {
        // The user moved the viewport or pressed a nav key while we
        // waited. Yanking them somewhere now would be worse than not
        // finishing the job.
        if (mine !== anchorToken) {
          console.log(tag, 'c: re-anchor cancelled, viewport moved');
          return;
        }
        const target = commentsTopTarget();
        if (target) {
          console.log(tag, `c: re-anchoring after open -> ${describeEl(target)}`);
          scrollToEl(target);
          return;
        }
        if (performance.now() < deadline) {
          ...
          console.log(tag, 'c: comments never appeared after opening');
        }
      })();
    }
    ...
    function jumpToComments() {
      ...
      if (!target) {
        if (spec.open && spec.open.canOpen()) {
          ...
          spec.open.click();
          if (spec.open.anchorAfterOpen) anchorAfterOpen();
          return;
        }
