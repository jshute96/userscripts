...
//   * Flat comments (no `parentOf` at all): every comment is a root
//     and they're all siblings, so `h`/`n`/`m` all become `j`, `l`
//     becomes `k`, and `p`/`r` have nowhere to go.
//
...
const CommentNav = (function () {
  ...
  // ---------------------------------------------------------------
  // `siblingsOf` and `rootsOf` call parentOf once per comment, so a
  ...
  function create(spec) {
    ...
    function siblingsOf(el, all) {
      const p = parentOf(el, all);
      return all.filter(c => parentOf(c, all) === p);
    }
    ...
    // `list` is either the full comment list or a derived one (this
    // comment's siblings, the thread roots) that `current` is always a
    // member of, so the not-found branch is defensive only.
    ...
    const ACTIONS = {
      ...
      siblingNext(all, current) {
        ...
        return step(siblingsOf(current, all), current, +1);
      },
      ...
      parentNext(all, current) {
        ...
        // Walk past the current subtree. From a reply that's the
        // parent's next sibling; from a root there's no subtree to
        // skip, so it's just the next root.
        const reference = parentOf(current, all) || current;
        return step(siblingsOf(reference, all), reference, +1);
      },
      root(all, current) {
        if (!current) return null;
      ...
      rootNext(all, current) {
        ...
        if (!current) return roots[0];
        return step(roots, rootOf(current, all), +1);
      },
    };
