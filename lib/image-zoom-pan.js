...
//   x                               Toggle between fit and original size (or last zoom)

// ImageZoomPan.create({
//   tag,               // log prefix; default '[image-zoom]'
//   viewport,          // selector: framed viewer's visible box
//   content,           // selector: element to transform; default img
//   img,               // selector: the <img>; default `${viewport} img`
//   fitToScreenClass,  // class on content when the app fits the image
//   canPlainDrag,      // (target, eventTarget) => bool: plain drag may pan
//   grabCursor,        // selector: shows grab cursor when zoomed; default 'img'
//   textField,         // selector: extra text fields to ignore keys in
//   scrollableOverlay, // selector: where plain wheel scrolls, not zooms
// }) -> { reset() } | null
//
// Installs zoom and pan on the framed viewer (if `viewport` is given)
// and on Chrome's standalone image viewer. Call once at startup.
// Returns null if neither applies, or if another script already
// installed it on this page.
//
// Which options to pass:
//   * Standalone image URLs only: none.
//   * A site with a viewer: `viewport` (the box that clips the image).
//     Pass `img` if the first <img> in `viewport` is the wrong one, and
//     `content` if something else has to move with the image (e.g. an
//     annotation layer drawn over it): then `content` is the element
//     wrapping both.
//   * The rest fix specific problems; add each only when you hit it:
//       fitToScreenClass   the app's own fit / actual-size toggle
//                          leaves the zoom wrong
//       canPlainDrag       the app uses plain drags on the image
//                          (drawing, cropping) and panning steals them
//       grabCursor         an overlay covers the <img>, so no grab
//                          cursor shows when zoomed
//       textField          typing in the app's custom inputs triggers
//                          zoom keys like `x` or `+`
//       scrollableOverlay  wheel over a popup or panel zooms instead of
//                          scrolling (popups with role dialog, listbox
//                          or menu already scroll)
//
// Also exported: ImageZoomPan.isStandaloneImageDoc() -> bool
const ImageZoomPan = (function () {
  ...
  const MIN_SCALE = 1;
  // Max zoom relative to 1x (fit), raised if needed to reach original size.
  const MAX_SCALE = 32;
  ...
  const DEFAULT_SCROLLABLE_OVERLAY_SEL = '[role="dialog"], [role="listbox"], [role="menu"]';
  ...
  function create(options) {
    ...
    // Prevent double-binding when both a site-specific viewer script
    ...
    // Whether the view is a zoomed fit-to-viewport (a small image enlarged
    // to fit), so a resize re-fits it. Unzoomed needs nothing: at 1x a
    // large image fits by CSS, and a small one is at original size.
    let atFit = false;
    // While zoomed: the real zoom (screen pixels per image pixel) and the
    // point of `content` (as fractions) at the viewport center. A resize
    // changes `content`'s fitted size and position, which `view` is
    // relative to, so onResize restores these instead of keeping `view`.
    let anchor = null;
    ...
    //   content  = element matching `contentSel`, or `img`
    ...
    function getTarget() {
      if (isStandaloneImageDoc()) {
        ...
        return { kind: 'standalone', viewport: body, content: img, img };
      }
      ...
      const content = contentSel ? document.querySelector(contentSel) : img;
      ...
      return { kind: 'framed', viewport, content, img };
    }
    ...
    // untransformed dimensions. The size comes from the transformed rect
    // divided by the scale, not `offsetWidth`/`offsetHeight`, which round
    // to whole pixels: at high zoom that rounding error is multiplied into
    // the offsets, and `x` then never gets back to exactly 1x. Because
    // `transform-origin` is `0 0`, the top-left corner is shifted only by
    // `translate(ox * w, oy * h)`.
    //
    // Measures against the transform actually on the element, which isn't
    // always `view`: Chrome's image viewer rewrites the <img>'s style on a
    // window resize, clearing our transform before onResize runs.
    function contentFrame(content) {
      const rect = content.getBoundingClientRect();
      const on = content.style.transform ? view : { s: 1, ox: 0, oy: 0 };
      const w = rect.width / on.s;
      const h = rect.height / on.s;
      return {
        x0: rect.left - on.ox * w,
        y0: rect.top - on.oy * h,
        w,
        h,
      };
    ...
    function pictureBox(target) {
      ...
      if (content === img) {
        ...
        return { relX: 0, relY: 0, picW: f.w, picH: f.h };
      }
      // Sub-pixel, untransformed box of `img` relative to `content`.
      const scale = content.style.transform ? view.s : 1;
      const cRect = content.getBoundingClientRect();
      const iRect = img.getBoundingClientRect();
      const boxW = iRect.width / scale;
      const boxH = iRect.height / scale;
      ...
      const relX = (iRect.left - cRect.left) / scale + (boxW - picW) / 2;
      const relY = (iRect.top - cRect.top) / scale + (boxH - picH) / 2;
    ...
    // With `center`, a picture that fits along this axis is centered on it.
    function clampAxis(o, s, x0, w, relStart, picSize, lo, hi, center) {
      if (w <= 0 || picSize <= 0) return 0;
      const z = s * picSize;
      if (z <= hi - lo && center) {
        return ((lo + hi - z) / 2 - x0 - s * relStart) / w;
      }
      if (z <= hi - lo) {
    ...
    // MAX_SCALE, or the scale of original size if that's bigger, so a huge
    // image (over 32x the viewport) can still reach one image pixel per
    // CSS pixel.
    function maxScale(target) {
      const natW = target.img.naturalWidth;
      const picW = pictureBox(target).picW;
      return natW > 0 && picW > 0 ? Math.max(MAX_SCALE, natW / picW) : MAX_SCALE;
    }

    function clampView(target, v) {
      const s = Math.min(maxScale(target), Math.max(MIN_SCALE, v.s));
      const f = contentFrame(target.content);
      const pic = pictureBox(target);
      const vp = target.viewport.getBoundingClientRect();
      // Chrome's image viewer keeps a picture that fits the window centered,
      // like at 1x. Framed viewers instead keep it under the cursor across
      // letterbox bars (see clampAxis).
      const center = target.kind === 'standalone';
      let ox = clampAxis(v.ox, s, f.x0, f.w, pic.relX, pic.picW, vp.left, vp.right, center);
      let oy = clampAxis(v.oy, s, f.y0, f.h, pic.relY, pic.picH, vp.top, vp.bottom, center);
      // Snap sub-pixel offsets to 0, so 1x is exactly unzoomed.
      if (Math.abs(ox * f.w) < 0.5) ox = 0;
      if (Math.abs(oy * f.h) < 0.5) oy = 0;
      return { s, ox, oy };
    }
    ...
    function applyView(target) {
      ...
      atFit = zoomed && sameView(target, view, fitView(target));
      if (zoomed) {
        const c = viewportCenter(target);
        anchor = { r: realZoom(target), ...toContentFraction(content, c.x, c.y) };
      } else {
        anchor = null;
      }
    }

    // Screen pixels per image pixel; 1 is original size.
    function realZoom(target) {
      const natW = target.img.naturalWidth;
      return natW > 0 ? (view.s * pictureBox(target).picW) / natW : view.s;
    }
    ...
    function zoomAt(target, factor, clientX, clientY) {
      absorbNativeScroll(target);
      const s = Math.min(maxScale(target), Math.max(MIN_SCALE, view.s * factor));
      if (s === view.s) return;
    ...
    // The view with the picture centered at scale `s`, clamped.
    function centeredView(target, s) {
      const pic = pictureBox(target);
      const f = contentFrame(target.content);
      const c = viewportCenter(target);
      return clampView(target, {
        s,
        ox: (c.x - f.x0 - s * (pic.relX + pic.picW / 2)) / f.w,
        oy: (c.y - f.y0 - s * (pic.relY + pic.picH / 2)) / f.h,
      });
    }

    // Fit to the viewport, and original size (one image pixel per CSS
    // pixel). At 1x, a large image is already fit and a small one is
    // already original size, so one of the two is always 1x.
    function fitView(target) {
      const pic = pictureBox(target);
      const vp = target.viewport.getBoundingClientRect();
      const s = Math.min(vp.width / pic.picW, vp.height / pic.picH);
      return s > 1 ? centeredView(target, s) : { s: 1, ox: 0, oy: 0 };
    }

    function originalView(target) {
      const pic = pictureBox(target);
      const s = target.img.naturalWidth / pic.picW;
      return s > 1 ? centeredView(target, s) : { s: 1, ox: 0, oy: 0 };
    }

    function sameView(target, a, b) {
      const f = contentFrame(target.content);
      return (
        Math.abs(a.s - b.s) < 0.01 * b.s &&
        Math.abs(a.ox - b.ox) * f.w < 1 &&
        Math.abs(a.oy - b.oy) * f.h < 1
      );
    }

    // `x`: toggle between fit-to-viewport and the last other view (the
    // original size, until the user zooms somewhere else).
    function toggleReset(target) {
      const pic = pictureBox(target);
      if (pic.picW <= 0 || pic.picH <= 0) return;
      const fit = fitView(target);
      if (sameView(target, view, fit)) {
        const next = savedView || originalView(target);
        if (!sameView(target, next, fit)) setView(target, next);
      } else {
        savedView = { ...view };
        setView(target, fit);
      }
    ...
    function inTextField(target) {
      ...
      return target.closest(textFieldSel) !== null;
    }
    ...
    // Whether plain left-drag and plain Shift+drag pan / box-zoom. A framed
    // viewer whose app uses plain drags itself (e.g. an annotation tool)
    // passes `opts.canPlainDrag` to say when they're free. Middle-drag and
    // Ctrl+drag always pan.
    function canPlainDrag(target, eventTarget) {
      if (target.kind === 'standalone' || !customCanPlainDrag) return true;
      return Boolean(customCanPlainDrag(target, eventTarget));
    }
    ...
    function ensureStyles() {
      ...
      const framedRules = viewportSel
        ...
        ${viewportSel}[${ZOOMED_ATTR}] :is(${grabCursorSel}) {
          cursor: grab;
        }
      `
        : '';
      style.textContent = `
        ...
        html[${STANDALONE_ATTR}] body {
          position: relative !important;
          /* With flex, the img's own margin: auto (Chrome's style, kept
             below) centers it both ways. justify-content / align-items
             don't center it in Chrome's image viewer. */
    ...
    function showHud(target) {
      ...
      const r = realZoom(target);
      // Thresholds on the rounded value, so 0.9999 shows as 1.0×, not 1.00×.
      hud.textContent = `${r.toFixed(r < 0.995 ? 2 : r < 9.95 ? 1 : 0)}×`;
    ...
    function onResize() {
      ...
      if (!target) return;
      if (atFit) {
        setView(target, fitView(target), false);
      } else if (anchor) {
        // Keep the pre-resize anchor, not the clamped result, so a window
        // shrunk and grown back returns to the same view.
        const keep = anchor;
        const natW = target.img.naturalWidth;
        const picW = pictureBox(target).picW;
        const s = natW > 0 ? (anchor.r * natW) / picW : view.s;
        const f = contentFrame(target.content);
        const c = viewportCenter(target);
        setView(target, {
          s,
          ox: (c.x - f.x0) / f.w - anchor.u * s,
          oy: (c.y - f.y0) / f.h - anchor.v * s,
        }, false);
        if (isZoomed(view)) anchor = keep;
      }
    }
